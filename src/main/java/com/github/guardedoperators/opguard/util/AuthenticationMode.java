...
public enum AuthenticationMode {
    ...
    public static Detected ofServer(Server server) {
        ...
        if (bungee) {
            return new Detected(paperBungeeCordMode(), ProxyType.BUNGEE);
        }
        if (isVelocityEnabled()) {
            return new Detected(paperVelocityMode(), ProxyType.VELOCITY);
        }
    ...
    private static boolean isBungeeCordEnabled(Server server) {
        try {
            return server.spigot().getConfig().getBoolean("bungeecord", false);
    ...
    public static class Detected {
        ...
        public Optional<ProxyType> proxy() {
